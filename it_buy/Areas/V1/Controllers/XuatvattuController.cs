...
namespace it_template.Areas.V1.Controllers
{
    ...
    public class XuatvattuController : BaseController
    {
        ...
        public XuatvattuController(ItContext context, QLSXContext qlsxcontext, IConfiguration configuration, UserManager<UserModel> UserMgr, ViewRender view) : base(context)
        {
        ...
        {
            var data = _QLSXContext.XuatVattuModel.Where(d => d.id == id)
        ...
        {
            var Model = _QLSXContext.XuatVattuModel.Where(d => d.id == id).FirstOrDefault();
        ...
        [HttpPost]
        public async Task<JsonResult> Save(XuatVattuModel XuatVattuModel, List<XuatVattuChitietModel> list_add, List<XuatVattuChitietModel>? list_update, List<XuatVattuChitietModel>? list_delete)
        {
            try
            {
                ...
                XuatVattuModel? XuatVattuModel_old;
                if (XuatVattuModel.date != null && XuatVattuModel.date.Value.Kind == DateTimeKind.Utc)
                {
                    XuatVattuModel.date = XuatVattuModel.date.Value.ToLocalTime();
                }
                if (XuatVattuModel.id == 0)
                {
                    int count = _QLSXContext.XuatVattuModel.Where(d => d.created_at.Value.Date == DateTime.Now.Date).Count();
                    count = count + 1;
                    var code = "XUAT_VATTU_" + DateTime.Now.Date.ToString("ddMMyy") + "_" + count.ToString("D3");
                    var check = _QLSXContext.XuatVattuModel.Where(d => d.code == code).FirstOrDefault();
                    if (check != null)
                    {
                        return Json(new { success = false, message = "Số HD đã tồn tại!" });
                    }
                    XuatVattuModel.code = code;
                    XuatVattuModel.created_at = DateTime.Now;
                    XuatVattuModel.status_id = (int)Status.Draft;
                    XuatVattuModel.created_by = user.Email;

                    _QLSXContext.XuatVattuModel.Add(XuatVattuModel);

                    _QLSXContext.SaveChanges();

                    XuatVattuModel_old = XuatVattuModel;

                }
                else
                {

                    XuatVattuModel_old = _QLSXContext.XuatVattuModel.Where(d => d.id == XuatVattuModel.id).FirstOrDefault();

                    CopyValues<XuatVattuModel>(XuatVattuModel_old, XuatVattuModel);
                    XuatVattuModel_old.updated_at = DateTime.Now;

                    _QLSXContext.Update(XuatVattuModel_old);
                    _QLSXContext.SaveChanges();
                }



                var list = new List<XuatVattuChitietModel>();
                if (list_delete != null)
                ...
                if (list_add != null)
                {
                    ...
                    {
                        item.xuat_id = XuatVattuModel_old.id;
                ...
                return Json(new { success = true, id = XuatVattuModel_old.id });
            }
            catch (Exception ex)
            {
        ...
        public async Task<JsonResult> xuatpdf(int id, bool is_view = false)
        {
            ...
            var data = _QLSXContext.XuatVattuModel.Where(d => d.id == id).Include(d => d.chitiet).FirstOrDefault();
            ...
            document.LoadFromFile(_configuration["Source:Path_Private"] + "/buy/templates/phiếu đề nghị xuất vật tư.docx", Spire.Doc.FileFormat.Docx);
            ...
            if (Directory.Exists(_configuration["Source:Path_Private"] + "\\buy\\xuatvattu\\" + id))
            {
                Directory.CreateDirectory(_configuration["Source:Path_Private"] + "\\buy\\xuatvattu\\" + id);
            }
            string url = "/private/buy/xuatvattu/" + id + "/" + timeStamp + ".docx";
            string url_pdf = "/private/buy/xuatvattu/" + id + "/" + timeStamp + ".pdf";
            ...
            {
                var output = ConvertWordFile(_configuration["Source:Path_Private"] + url.Replace("/private", "").Replace("/", "\\"), _configuration["Source:Path_Private"] + "\\buy\\xuatvattu\\" + id);
                //if (process != "Success")
                if (output == false)
                {
            ...
            else
            {
                ...
                {
                    name_vi = "Đề nghị xuất vật tư " + data.code,
        ...
        public async Task<JsonResult> Table()
        {
            ...
            var customerData = _QLSXContext.XuatVattuModel.Where(d => d.deleted_at == null);
        ...
        //    //var tenhh = Request.Form["filters[tenhh]"].FirstOrDefault();
        //    int skip = start != null ? Convert.ToInt32(start) : 0;
        //    var customerData = _QLSXContext.XuatVattuChitietModel
        ...
        //    Expression<Func<XuatVattuChitietModel, bool>> filter = p => p.dutru.created_by == user_id; // Biểu thức ban đầu là true (chấp nhận tất cả)
        ...
        //        var listdutru = _QLSXContext.XuatVattuModel.Where(d => list_dt.Contains(d.code)).Select(d => d.id).ToList();
