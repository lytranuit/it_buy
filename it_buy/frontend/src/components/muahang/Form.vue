...
const { model, list_add, list_update, list_delete, datatable, waiting } =
...
const submit = async () => {
  ...
  if (response.success) {
    ...
    if (model.value.is_sample) {
      await muahangApi.saveHangmau({
        muahang_id: response.id,
        nhacungcap_id: model.value.nhacungcap_id,
      });
    }
