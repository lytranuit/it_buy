    ...
    <Dialog v-model:visible="visibleDialog" :header="headerForm" :modal="true" class="p-fluid">
        <div class="row mb-2">
            <div class="field col">
                ...
                <InputText id="name" class="p-inputtext-sm" v-model.trim="model.mansx" required="true"
                    :class="{ 'p-invalid': submitted && !model.mansx }" />
                <small class="p-error" v-if="submitted && !model.mansx">Required.</small>
            </div>
            <div class="field col">
                <label for="name">Tên<span class="text-danger">*</span></label>
                <InputText id="name" class="p-inputtext-sm" v-model.trim="model.tennsx" required="true"
                    :class="{ 'p-invalid': submitted && !model.tennsx }" />
                <small class="p-error" v-if="submitted && !model.tennsx">Required.</small>
            </div>
        </div>
        <template #footer>
