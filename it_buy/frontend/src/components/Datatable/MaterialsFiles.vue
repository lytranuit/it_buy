...
const { files, model, waiting } = storeToRefs(store_materials);
...
const confirmDeleteDinhkem = (item) => {
  ...
  confirm.require({
    ...
    accept: () => {
      ...
      materialsApi.xoadinhkem({ list_id: list_id }).then((response) => {
        waiting.value = false;
        if (response.success) {
          load();
        }
...
const save = () => {
  ...
  waiting.value = true;
  hideDialog();
  materialsApi.saveDinhkem(params).then((response) => {
    waiting.value = false;
    if (response.success) {
      toast.add({
        severity: "success",
