    ...
    <DataTable class="p-datatable-customers" showGridlines :value="datatable" :lazy="true" ref="dt" scrollHeight="70vh"
        ...
        <Column v-for="col of selectedColumns" :field="col.data" :header="col.label" :key="col.data"
            ...
            <template #body="slotProps">
                <template v-if="col.data == 'id' && user.email == slotProps.data.created_by">
                    <RouterLink :to="'/vattu/phieuxuat/edit/' + slotProps.data[col.data]">
        ...
        <Column style="width: 1rem">
            <template #body="slotProps">
                <a class="p-link text-danger font-16" @click="confirmDelete(slotProps.data['id'])"
                    v-if="user.email == slotProps.data.created_by"><i class="pi pi-trash"></i></a>
            </template>
        </Column>
    </DataTable>
</template>
...
const showing = ref([]);
const column_cache = "columns_phieuxuat";
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
    loading.value = true;
    VattuApi.tableXuat(lazyParams.value).then((res) => {
