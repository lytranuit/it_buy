<template>
  <DataTable class="dt-responsive-table" showGridlines :value="datatable" :lazy="true" ref="dt" scrollHeight="70vh"
      ...
      <div class="d-md-flex align-items-center">
        ...
        <div class="ml-auto">
          <SelectButton v-model="filterTable" :options="list_filterTable" aria-labelledby="basic"
            :pt="{ button: 'form-control-sm' }" @change="loadLazyData" optionValue="value" :allowEmpty="true">
        ...
        <div class="ml-3">
          <SelectButton v-model="filterTable1" :options="list_filterTable1" aria-labelledby="basic"
            :pt="{ button: 'form-control-sm' }" @change="loadLazyData" optionValue="value" :allowEmpty="true">
            <template #option="slotProps">
              {{ slotProps.option.label }}
            </template>
        ...
        <div class="ui-column-data">
          ...
          <template v-else-if="col.data == 'name'">
            <div>
              <div style="text-wrap: pretty">
                <router-link :to="'/dutru/edit/' + slotProps.data.id" class="text-blue">{{ slotProps.data.name }}
                </router-link>
              </div>
              <small>Tạo bởi
          ...
          <template v-else-if="col.data == 'list_muahang'">
            <div v-for="item of slotProps.data[col.data]" :key="item.id">
              ...
              <RouterLink :to="'/muahang/nhanhang/' + item.id" class="text-primary mr-2" v-else-if="
                item['is_dathang'] &&
          ...
          <template v-else-if="col.data == 'priority_id'">
            <div class="text-center">
              <Tag value="Bình thường" size="small" v-if="slotProps.data['priority_id'] == 1" />
              <Tag value="Ưu tiên" severity="warning" size="small" v-else-if="slotProps.data['priority_id'] == 2" />
              <Tag value="Gấp" severity="danger" size="small" v-else-if="slotProps.data['priority_id'] == 3" />
            </div>
          </template>
    ...
    <Column style="width: 1rem">
      <template #body="slotProps">
        <a class="p-link text-warning font-16 mr-2" @click="confirmCopy(slotProps.data['id'])"
          v-if="slotProps.data.created_by == user.id"><i class="pi pi-copy"></i></a>
