...
const { model, datatable, list_add } = storeToRefs(storexuatnguyenlieu);
onMounted(() => {
  storexuatnguyenlieu.reset();
...
const submit = () => {
  ...
  xuatnguyenlieuApi.save(params).then((response) => {
    messageError.value = "";
    if (response.success) {
      toast.add({
        severity: "success",
      ...
      router.push("/xuatnguyenlieu/edit/" + response.id);
    } else {
