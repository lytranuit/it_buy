...
const submit = async () => {
  ...
  waiting.value = true;
  var response = await xuatvattuApi.save(params);
  waiting.value = false;
  if (response.success) {
    toast.add({
      severity: "success",
...
const view = async () => {
  await submit();
  waiting.value = true;
  var response = await xuatvattuApi.xuatpdf(model.value.id, true);
  waiting.value = false;
  if (response.success) {
...
const xuatpdf = async () => {
  await submit();
  waiting.value = true;
  var response = await xuatvattuApi.xuatpdf(model.value.id);
