  ...
  <div class="row clearfix">
    ...
    >
      <div class="confirmation-content">
        ...
        <span v-if="model"
          >Bạn có muốn xóa <b>{{ model.mancc }}</b> này không?</span
        >
      </div>
...
const filters = ref({
  ...
  tenncc: { value: null, matchMode: FilterMatchMode.CONTAINS },
});
const totalRecords = ref(0);
const loading = ref(true);
const showing = ref([]);
const column_cache = "columns_supplier"; ////
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
  loading.value = true;
  supplierApi.table(lazyParams.value).then((res) => {
    // console.log(res);
